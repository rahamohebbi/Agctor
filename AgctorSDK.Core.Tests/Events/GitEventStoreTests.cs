...
{
    public class GitEventStoreTests : IDisposable
    {
        ...
        public async Task RecordAsync_ShouldCreateFileAndCommitCorrectly_WithCli()
        {
            ...
            // Assert
            Assert.True(await GitCliHelper.IsGitRepositoryAsync(repoPath), "Repository should be initialized by Git CLI.");
            ...
            Assert.True(File.Exists(expectedEventFile), "Event JSON file should exist.");
            ...
            Assert.Equal(record.Id, deserializedRecord.Id);
            Assert.Equal(record.EventType, deserializedRecord.EventType);
            ...
            Assert.NotNull(latestCommitHash);
        ...
        public async Task QueryAsync_ShouldReturnMatchingEventsByPromptHash_WithCli()
        {
            ...
            Assert.Contains(results, r => r.Id == record1.Id);
            Assert.Contains(results, r => r.Id == record3.Id);
            Assert.Equal(record1.Id, results[0].Id);
            Assert.Equal(record3.Id, results[1].Id);
        }
        ...
        public async Task RecordAsync_WithNullRecord_ShouldThrowArgumentNullException_WithCli()
        {
            ...
            await Assert.ThrowsAsync<ArgumentNullException>(() => eventStore.RecordAsync(null!));
        }
        ...
        public async Task RecordAsync_WithInvalidEventId_ShouldThrowArgumentException_WithCli(string invalidId)
        {
            ...
            await Assert.ThrowsAsync<ArgumentException>(() => eventStore.RecordAsync(record));
        }
