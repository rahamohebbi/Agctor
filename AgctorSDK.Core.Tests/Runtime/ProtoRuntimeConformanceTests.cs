...
/// <summary>
/// PRD-020 stable-subset checks for the experimental Proto.Actor adapter. Tests
/// here intentionally cover only behavior that is stable enough today; skipped
/// tests document known drift from the InMemory supported baseline.
/// </summary>
public sealed class ProtoRuntimeConformanceTests : IDisposable
{
    private readonly ProtoActorAdapter _runtime = new();
    ...
    public async Task Runtime_Spawns_And_Resolves_Actor()
    {
        ...
        var actor = await _runtime.SpawnActorAsync("proto-conf-actor", id => new ConformanceActor(id));
        var resolved = await _runtime.GetActorAsync<ConformanceActor>("proto-conf-actor");
    ...
    {
        await _runtime.InitializeAsync(new Dictionary<string, object>());
        await _runtime.SpawnActorAsync("proto-conf-request", id => new ConformanceActor(id));
    ...
    {
        await _runtime.InitializeAsync(new Dictionary<string, object>());
        var actor = await _runtime.SpawnActorAsync("proto-conf-send", id => new ConformanceActor(id));
        ...
        received.Headers[AgctorMessageHeaders.ReceiverId].Should().Be("proto-conf-send");
    ...
    {
        private readonly TaskCompletionSource<IMessageEnvelope> _received =
        ...
        {
            if (envelope.Payload.GetType().Name != "Started")
            {
