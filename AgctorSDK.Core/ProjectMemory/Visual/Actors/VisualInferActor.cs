...
/// <summary>Gemma 4 vision subject/intent inference (PRD-023d).</summary>
public sealed class VisualInferActor : IActor
{
    private readonly VisualPipelineService _pipeline;
    private ActorState _state = ActorState.Initializing;

    public VisualInferActor(string id, VisualPipelineService pipeline)
    {
    ...
    public string ActorType => nameof(VisualInferActor);
    ...
    public async Task<IMessageEnvelope> ReceiveAsync(IMessageEnvelope envelope, CancellationToken cancellationToken = default)
    {
        try
        {
            if (envelope.Payload is VisualInferRequest req)
            {
                var result = await _pipeline.InferFromPromptAsync(req, cancellationToken).ConfigureAwait(false);
            ...
            return AgctorEnvelopeBuilder.Error(
                envelope,
                Id,
                $"Unsupported visual infer payload '{envelope.Payload?.GetType().Name ?? "null"}'.");
        }
        catch (Exception ex)
        {
